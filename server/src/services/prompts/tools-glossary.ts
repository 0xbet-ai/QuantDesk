...
- \`mcp__quantdesk__request_validation({runId?})\` — dispatch Risk Manager validation on a specific run. Pass \`runId\` whenever the user names a non-latest run; omit it only when the latest run is the intended target. Requires prior user consent. **Call once, then end your turn.** The RM runs asynchronously; you will be retriggered with the verdict. Do not call again while waiting.
