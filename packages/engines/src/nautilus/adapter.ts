...
export class NautilusAdapter implements EngineAdapter {
	...
	async downloadData(config: DataConfig): Promise<DataRef> {
		// Nautilus uses the agent-written runner to fetch data. We just ensure
		// the target directory exists; the runner.py is responsible for
		// populating it during runBacktest.
		return {
