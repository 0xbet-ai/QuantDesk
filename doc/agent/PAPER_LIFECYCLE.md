...
How a desk transitions from a validated backtest into a long-running paper trading session, how the agent participates while the session is alive, and how the server reconciles paper containers across restarts. For the turn-based backtest lifecycle and MCP tool contract see `./TURN.md` and `./MCP.md`.
...
- **No automatic restart, ever.** A crashed paper session stays `failed` until the user promotes a new run — same principle as the backtest failure flow in `./TURN.md`.
