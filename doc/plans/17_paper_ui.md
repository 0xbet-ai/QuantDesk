...
1. Agent calls `mcp__quantdesk__go_paper({runId})` after the user has agreed in a previous turn → `paperSessions` row created (round-trip integration test).
...
4. The header pending-decision badge from rule #12 still shows for any unresolved proposal alongside the paper widget.
