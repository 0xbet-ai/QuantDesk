...
Spec: CLAUDE.md rule #10 ("Paper sessions must survive server restart").
...
2. Given a `running` row whose container is gone, the reconciler transitions it to `failed` with a reason and posts a rule #12 system comment ("Container exited. Reply to investigate.").
