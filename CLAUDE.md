...
7. **Managed engine whitelist is closed.** The set of managed engine adapters we ship is fixed and defined in `doc/engine/README.md`. No additional managed adapters will be added. A generic fallback engine exists for venues that do not map to any managed engine and runs agent-authored scripts inside a pinned Ubuntu+Python container.
8. **Strategy mode selection** — the onboarding wizard asks the user to pick a strategy mode **between venue selection and strategy selection**. When a venue supports multiple modes, the mode choice decides which engine is used. When a venue only supports one mode, others are disabled for that venue. See `doc/engine/README.md` for the mode catalog and per-mode capabilities.
9. **Paper trading uses engine-native paper modes for managed engines** — never build a custom paper trading simulator for a managed engine. Generic desks run agent-authored paper loop scripts inside the generic container, because there is no "native" engine to defer to — the agent's script is the engine. Per-engine paper configuration details live in `doc/engine/README.md`.
...
11. **Engines run in Docker** — all engine processes (managed backtest, managed paper, generic backtest, generic paper) execute inside Docker containers using images with **pinned version tags** (never `:latest`). Never install any engine natively on the host. Rationale: isolation of LLM-generated strategy code, reproducibility, simplified user setup. The server, UI, and Claude/Codex CLI run on the host — only the engine layer is containerized. Image pinning details and per-engine image choices live in `doc/engine/README.md`.
12. **Paper session recovery** — paper runs may live for days. Containers are tagged with `quantdesk.*` labels so the server reconciles running containers on restart via `docker ps` rather than marking them as failed. The in-memory registry is rebuilt from Docker as the source of truth. Label set and reconcile mechanism: `doc/engine/README.md` and `doc/agent/PAPER_LIFECYCLE.md`.
13. **First-run data fetch is agent-proposed, user-approved.** For any brand-new desk with no strategy code and no registered dataset, the agent's FIRST response must be a `[PROPOSE_DATA_FETCH] {...} [/PROPOSE_DATA_FETCH]` block containing `{exchange, pairs, timeframe, days, tradingMode, rationale}`. The agent must then stop and wait — it must not write code or emit `[RUN_BACKTEST]` until a user-approved fetch has completed. The UI renders this proposal as inline Approve/Reject buttons. On approve, the server runs the engine-specific download inside a container (per-engine download tool is documented in `doc/engine/README.md`), registers a Dataset row, and posts a system comment that re-triggers the agent to continue with strategy authoring and `[RUN_BACKTEST]`. On reject, the agent is re-triggered so it can propose a revised dataset. The server does **not** silently auto-download — the agent owns the decision of venue/pair naming/timeframe/history window, and the user owns the approval.
