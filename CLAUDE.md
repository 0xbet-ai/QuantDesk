...
7. **Managed engines: Freqtrade and Nautilus Trader only.** QuantDesk ships first-class managed adapters for these two engines only — no managed Hummingbot adapter will be added. (Users are free to `pip install hummingbot` inside a generic-mode script if they want; we simply don't ship or maintain a dedicated adapter for it.) Generic engine remains as a fallback for venues with no managed engine. Like managed engines, the generic engine runs inside a pinned Docker image (Ubuntu base + Python) — never on the host. Generic supports both backtest and paper trading: the agent writes the strategy script, the fetch script, and the paper loop script, all executed inside the generic container.
